...
ggplot(df_LacksSelfConfidence_is_5, aes(x = ClosedCommunication, fill = Gender)) +
  geom_density(alpha = 0.7, size = 0.5, color = "gray35") +
...
```{r,echo=TRUE}
df <- df %>%
  mutate(LacksSelfConfidence_is_5 = ifelse(LacksSelfConfidence == 5, "True", "False"))
...
ggplot(cond_male, aes(x = ClosedCommunication, fill = LacksSelfConfidence_is_5)) +
  ...
  scale_fill_manual(values = c("False" = "navyblue", "True" = "seagreen3")) +
  theme_minimal()
```
